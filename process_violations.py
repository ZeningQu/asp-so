...
def main():
    with open(absolute_path('weights.lp')) as weight_constants, open(absolute_path('assign_weights.lp'), 'w') as assign, open(absolute_path('weights.json'), 'w') as weights_json:
        ...
        for line in weight_constants.readlines():
            ...
            if match:
                ...
                assign.write(f'violation_weight({name},{name}_weight).\n')
